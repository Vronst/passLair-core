...
def make_password_manager(mock_user_manager: MagicMock) -> PasswordManager:
...
class TestInit:
    def test_wraps_reader_and_writer_for_given_user(self, mock_user_manager: MagicMock):
        manager = PasswordManager(mock_user_manager)
...
class TestGetPasswordForService:
    ...
    def test_not_found_reports_failure(self, mock_user_manager: MagicMock):
        manager = make_password_manager(mock_user_manager)
...
class TestSetPasswordForService:
    ...
    def test_unhandled_exception_propagates(self, mock_user_manager: MagicMock):
        ...
        with pytest.raises(ArithmeticError):
            _ = manager.set_password_for_service("github.com", "bob", "hunter2")
