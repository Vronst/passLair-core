...
def test_init_sqlite_default_path(db):
...
def test_init_mariadb_pool_configurations(db):
...
def test_session_context_manager_uninitialized_error(db):
...
def test_session_successful_commit(db):
